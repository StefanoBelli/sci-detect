...
static int __write_via_store_instr(char *mem)
{
	spurious_byte_memwrite(mem, 'a');
	return 0;
}

static int chld1_dowrite_shared_filepage(char *mem, void *args)
{
	int rv = EXIT_SUCCESS;
...
int main()
{
	...
	/* big-file (10-page-sized file): fork - nonpresent, store instr, read after */
	{
		...
		__test_fork_and_wait(__child_base(
					chld1_dowrite_shared_filepage, page_nr(1), __write_via_store_instr));
	...
	/* big-file (10-page-sized file): fork - nonpresent, syscall, write after */
	{
		...
		__test_fork_and_wait(__child_base(
					chld1_dowrite_shared_filepage, page_nr(1), __write_via_syscall));
